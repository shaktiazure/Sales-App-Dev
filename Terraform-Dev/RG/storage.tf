...
/*resource "azurerm_container_app_environment" "sdevContainer" {
  name                = "devtfstate01"
...
resource "azurerm_storage_container" "sdevContainer" {
  name                  = "devtfstate"
  storage_account_name  = azurerm_storage_account.sdevstorage.name
  container_access_type = "private"
}

resource "azurerm_storage_account" "sdevstorage" {
  name                     = "salesdevstorage"
  resource_group_name      = azurerm_resource_group.devRG.name
  location                 = azurerm_resource_group.devRG.location
